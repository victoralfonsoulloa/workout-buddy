// workout.jsx — active workout session screen

const { useState: useStateW, useMemo: useMemoW } = React;

function makeInitialSets(exercises) {
  const map = {};
...
function NumStepper({ value, onChange, step = 2.5, suffix }) {
  return (
    <div style={{
...
function SetRow({ setIdx, set, prevWeight, prevReps, onChange, onComplete }) {
  const isPR = set.done && set.weight > prevWeight;
  return (
    <div style={{
      display: 'grid',
...
function ExerciseCard({ exercise, sets, onSetChange, onSetComplete, expanded, onToggle, idx }) {
...
function PRCelebration({ visible, exerciseName, weight, reps, onDismiss }) {
...
function WorkoutScreen({ onFinish, onCancel }) {
  const D = window.WB_DATA;
  const split = D.getNextSplit();
  const [setMap, setSetMap] = useStateW(() => makeInitialSets(split.exercises));
  const [expandedId, setExpandedId] = useStateW(split.exercises[0].id);
  const [pr, setPr] = useStateW(null);

  const totalSets = useMemoW(() => split.exercises.reduce((acc, ex) => acc + ex.sets, 0), [split]);
  const doneSets = useMemoW(
    () => Object.values(setMap).flat().filter(s => s.done).length,
    [setMap]
  );
  const progress = doneSets / totalSets;

  const updateSet = (exId, idx, newSet) => {
    setSetMap(m => ({ ...m, [exId]: m[exId].map((s, i) => i === idx ? newSet : s) }));
  };
  const completeSet = (ex, idx) => {
    const cur = setMap[ex.id][idx];
    if (cur.done) return;
    const newSet = { ...cur, done: true };
    setSetMap(m => ({ ...m, [ex.id]: m[ex.id].map((s, i) => i === idx ? newSet : s) }));
    // PR if weight > lastWeight (and weight > 0)
