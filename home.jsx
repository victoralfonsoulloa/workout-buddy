...
function HeroCard({ split, motivation, streak, onStart, heroStyle }) {
  // heroStyle: 'gradient' | 'typographic' | 'illustration'
  const renderArt = () => {
...
function MotivationCard({ phrase }) {
  return (
    <div style={{
  ...
  return (
    <Card padding={18}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {data.days.map((d, i) => {
          let bg = 'var(--wb-bg)', fg = 'var(--wb-ink-subtle)', label = d.day;
...
function RecentSession({ session, onClick }) {
  const splitColors = {
...
function HomeScreen({ onStart, onSelectSession, heroStyle }) {
  const D = window.WB_DATA;
  const split = D.getNextSplit();
  const motivation = D.getMotivation();

  return (
    <div className="wb-scroll">
      <ScreenHeader
        greeting="Wednesday · Apr 29"
        title="Hey, Alex 👋"
      />

      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        <MotivationCard phrase={motivation} />
        <HeroCard split={split} motivation={motivation} streak={D.STREAK} onStart={onStart} heroStyle={heroStyle} />
        <WeekProgress data={D.THIS_WEEK} />
        ...
        <div style={{ marginTop: 6 }}>
          ...
          <Card padding={'0 18px'}>
            {D.HISTORY.slice(0, 4).map((s, i, arr) => (
