// history.jsx — History screen + session detail

const { useState: useStateHi } = React;
    ...
    <div className="wb-tap" onClick={onClick} style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 10, fontSize: 11.5, color: 'var(--wb-ink-muted)' }}>
          <span><Icon name="clock" size={11} style={{ marginRight: 3, verticalAlign: '-2px' }} />{session.duration}</span>
...
function HistoryScreen({ selectedSession, onSelectSession, onBack }) {
  ...
  // Group: This week (Apr 27+), Last week, Earlier
  const groups = [
    { label: 'This week', items: D.HISTORY.filter(s => ['Apr 27','Apr 25'].includes(s.dateShort)) },
    { label: 'Last week', items: D.HISTORY.filter(s => ['Apr 22','Apr 20','Apr 18'].includes(s.dateShort)) },
    { label: 'Earlier', items: D.HISTORY.filter(s => ['Apr 15'].includes(s.dateShort)) },
  ].filter(g => g.items.length > 0);

  // Top stats
  const totalSessions = D.HISTORY.length;
  const totalPRs = D.HISTORY.filter(s => s.pr).length;
  const totalVolume = D.HISTORY.reduce((a, s) => a + s.volume, 0);
